...
def test_assemble_stacks_in_order() -> None:
...
def test_assemble_skips_the_copy_for_a_single_request() -> None:
...
def test_assemble_handles_multi_row_requests() -> None:
    batcher = StackingBatcher(INPUTS, OUTPUTS, max_batch_size=8)
...
def test_scatter_returns_each_request_its_own_rows() -> None:
...
def test_scatter_produces_views_not_copies() -> None:
    batcher = StackingBatcher(INPUTS, OUTPUTS, max_batch_size=8)
...
def test_scatter_rejects_a_row_count_mismatch() -> None:
...
def test_assemble_rejects_an_oversized_batch() -> None:
...
def test_assemble_rejects_a_wrong_shape() -> None:
    batcher = StackingBatcher(INPUTS, OUTPUTS, max_batch_size=8)
    request = InferenceRequest(
