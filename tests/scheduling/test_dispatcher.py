...
class TestPolicyPlacement:
    """The dispatcher honours the policy's choice, over the instances it may actually use."""

    def test_dispatch_places_on_the_policy_choice(self) -> None:
    ...
    def test_dispatch_skips_instances_that_are_not_ready(self) -> None:
...
class TestSpillover:
    """A full first choice becomes a second attempt; only a saturated pool becomes an error."""

    def test_dispatch_spills_when_the_first_choice_is_full(self) -> None:
    ...
    def test_dispatch_raises_only_when_the_whole_pool_is_saturated(self) -> None:
...
class TestNoUsableInstance:
    """With nowhere to dispatch to, the dispatcher fails loudly rather than dropping work."""

    def test_dispatch_raises_when_nothing_is_ready(self) -> None:
        instances = [FakeInstance(Device.cuda(i), is_ready=False) for i in range(2)]
        dispatcher = Dispatcher("m", instances, RoundRobinPolicy())
        with pytest.raises(ServerStateError):
            dispatcher.dispatch(_item(), _enqueue)

    def test_dispatcher_refuses_to_exist_without_instances(self) -> None:
        with pytest.raises(ServerStateError):
