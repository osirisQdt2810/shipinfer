...
class TestDelayWindow:
    """How long a batch is allowed to wait: full now, or hold the window open for stragglers."""

    def test_returns_immediately_when_batch_is_already_full(self, make_item) -> None:
    ...
    def test_waits_for_the_window_then_sends_a_partial_batch(self, make_item) -> None:
        queue = FairPriorityQueue("q", capacity=16)
    ...
    def test_late_arrivals_join_the_batch(self, make_item) -> None:
...
class TestPreferredSizes:
    """Rounding a batch down to a profiled shape, in the queue and in the sizing function."""

    def test_preferred_size_short_circuits_the_wait(self, make_item) -> None:
        queue = FairPriorityQueue("q", capacity=16)
        for i in range(4):
            queue.put(make_item(frame=i))
    ...
    def test_choose_batch_size(self, size, preferred, maximum, expected) -> None:
        assert choose_batch_size(size, preferred, maximum) == expected


class TestBatchWindowValidation:
    """Preferred sizes that can never be reached are a config error, not a surprise."""

    def test_window_rejects_impossible_preferred_sizes(self) -> None:
