...
def test_join_shortest_queue_picks_the_shortest() -> None:
    instances = [
...
def test_power_of_two_never_picks_the_same_instance_twice() -> None:
...
def test_locality_keeps_work_on_the_resident_gpu() -> None:
    instances = [FakeInstance(Device.cuda(0), depth=2), FakeInstance(Device.cuda(1), depth=0)]
    policy = LocalityAwareSpilloverPolicy(spill_threshold=4)
    chosen = policy.select(instances, _request(resident=Device.cuda(0)))
    assert chosen.device.index == 0, "a 6 MB frame should not cross PCIe to save 2 queue slots"


def test_locality_spills_once_the_resident_gpu_backs_up() -> None:
    instances = [FakeInstance(Device.cuda(0), depth=10), FakeInstance(Device.cuda(1), depth=0)]
    policy = LocalityAwareSpilloverPolicy(spill_threshold=4)
    chosen = policy.select(instances, _request(resident=Device.cuda(0)))
    assert chosen.device.index == 1


def test_locality_falls_back_when_there_is_no_hint() -> None:
    instances = [FakeInstance(Device.cuda(0), depth=5), FakeInstance(Device.cuda(1), depth=0)]
    policy = LocalityAwareSpilloverPolicy(spill_threshold=4, fallback=JoinShortestQueuePolicy())
    assert policy.select(instances, _request()).device.index == 1


def test_sequence_affinity_pins_a_camera() -> None:
...
def test_sequence_affinity_repins_when_the_instance_dies() -> None:
...
def test_every_policy_is_registered_and_buildable() -> None:
...
def test_unknown_policy_names_its_alternatives() -> None:
