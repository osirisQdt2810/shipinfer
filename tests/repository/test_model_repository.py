...
def test_scans_the_demo_repository(demo_repository_path: Path) -> None:
...
def test_resolve_defaults_to_the_latest_version(tmp_path: Path) -> None:
...
def test_version_policy_limits_what_is_loadable(tmp_path: Path) -> None:
...
def test_unknown_model_lists_what_is_loaded(tmp_path: Path) -> None:
    repo = ModelRepository.load(_repo_with_versions(tmp_path, [1]))
    with pytest.raises(ModelNotFoundError, match="loaded: \\['m'\\]"):
        repo.entry("nope")


def test_directory_name_is_authoritative(tmp_path: Path) -> None:
...
def test_a_triton_config_is_recognised_and_explained(tmp_path: Path) -> None:
    root = tmp_path / "repo"
...
def test_missing_version_directory_is_an_error(tmp_path: Path) -> None:
    root = tmp_path / "repo"
...
def test_ensembles_need_no_version_directory(demo_repository_path: Path) -> None:
    repo = ModelRepository.load(demo_repository_path)
...
def test_artifact_file_lists_what_is_present(tmp_path: Path) -> None:
