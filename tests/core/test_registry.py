...
class TestRegistration:
    """Registering a class makes it retrievable by name, alias and description."""
    ...
    def test_description_defaults_to_the_docstring_summary(self) -> None:
        registry: Registry[Base] = Registry("thing", Base)
...
class TestRegistryErrors:
    """A misuse of the registry fails loudly at import time, naming the fix."""

    def test_registration_enforces_the_base_class(self) -> None:
...
class TestLazyRegistration:
    """A lazily registered entry can be listed without importing it."""

    def test_lazy_registration_defers_the_import(self) -> None:
    ...
    def test_lazy_registration_reports_a_missing_module_usefully(self) -> None:
        registry: Registry[Base] = Registry("thing", Base)
