...
class TestTheModelTableIsNeverIteratedLive:
    """`unload_model` pops from `_models` on a request thread, and FastAPI runs the plain-`def`
    handlers in its threadpool — so a scrape and an unload really do interleave.

    The failure is not a stale answer. It is `RuntimeError: dictionary changed size during
    iteration` from an endpoint that worked a second earlier, and `is_ready` is the sharpest
    case: a 500 there takes the pod out of rotation.

    WHAT IS TESTED HOW, AND WHY THEY DIFFER — because two attempts at this were worse than
    useless and the reason is worth keeping.

    `__iter__` is checked *deterministically*: it hands the caller an iterator, so holding one
    across an unload needs no timing at all, and that test fails on the unfixed code every run.

    `is_ready` and `stats` are checked *against the source* instead. Two harnesses were tried
    and both were rejected. A single-threaded fake — a dict whose `values()` pops mid-iteration
    — raises whether or not the read is locked, so it cannot tell the fix from its absence. A
    genuine two-thread hammer of 2000 reads against a churning loader did not reproduce once on
    the unfixed engine: the interleave is real but its window is small, and a threaded test that
    passes on broken code is worse than no test, because it reads as coverage.

    So the guarantee here is structural — every reader goes through one snapshot helper — and
    that is asserted by reading the class. It is a weaker kind of evidence than a failing run
    and it is the strongest available; saying so is better than a green hammer.
    """

    def _loaded(self, explicit: InferenceServer, count: int = 2) -> list[str]:
        for name in ("echo", "other")[:count]:
            explicit.load_model(name)
        return explicit.models()

    def test_iterating_the_server_hands_back_a_snapshot(
        self, explicit: InferenceServer
    ) -> None:
        """The one a careful caller was compensating for: `check_health` does `list(server)`,
        and being correct only for callers who remember to is not a property."""
        names = self._loaded(explicit)
        walker = iter(explicit)
        next(walker)

        explicit.unload_model(names[-1])
