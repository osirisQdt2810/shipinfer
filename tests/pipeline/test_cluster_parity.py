"""The Python half of the cluster golden: is the committed file still what the reference says?

The C++ binary (`csrc/tests/test_tracking_cluster_parity.cpp`) is the real gate -- it is the
port that can drift. This one answers the other question, which that binary cannot: whether the
GOLDEN still matches the reference it was emitted from, so a submodule bump that changes a label
or a threshold is caught here rather than showing up as "the port is wrong".
...
from benchmarks.parity.drive_cluster import GOLDEN, SCENARIOS, load, render_cluster
...
class TestTheGoldenIsStillTheReferencesAnswer:
    ...
    def test_the_golden_is_reproduced_exactly(self, name: str) -> None:
        ...
        assert render_cluster(load(name)) == expected, (
            f"the ids the reference assigns for {name} are not the committed golden. "
            f"If a submodule bump IS the decision, re-emit with `python "
            f"scripts/emit_parity_golden.py --kind cluster --scenario {name} --emit-golden "
            f"--force`, say so in the PR, and check the C++ cluster in the same commit"
        )

    @pytest.mark.parametrize("name", NAMES)
    def test_the_golden_names_the_command_that_emits_it(self, name: str) -> None:
        """The C++ cluster asserts this too. Both, because a golden nobody can regenerate is one
        that gets hand-edited the first time it fails."""
        text = (GOLDEN / f"{name}.txt").read_text(encoding="utf-8")

        assert "emit_parity_golden.py --kind cluster" in text
...
class TestTheCppGateReadsTheSameFiles:
    def test_the_binary_names_this_scenario_and_this_golden(self) -> None:
        ...
        binary = "test_tracking_cluster_parity.cpp"
        source = (Path(__file__).resolve().parents[2] / "csrc" / "tests" / binary).read_text(
            encoding="utf-8"
        )

        for name in NAMES:
            assert f"scenarios/cluster/{name}.txt" in source
            assert f"golden/cluster/{name}.txt" in source
