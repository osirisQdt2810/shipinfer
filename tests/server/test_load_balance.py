...
def test_every_instance_receives_work(tmp_path: Path, policy: str) -> None:
...
def test_a_skewed_camera_cannot_starve_the_others(tmp_path: Path) -> None:
...
def test_backpressure_rejects_rather_than_evicting(tmp_path: Path) -> None:
