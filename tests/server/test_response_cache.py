...
def test_identical_inputs_run_the_model_once(tmp_path: Path) -> None:
    ...
    with _server(_repo(tmp_path)) as server:
        ...
        np.testing.assert_array_equal(first.outputs["y"].numpy(), second.outputs["y"].numpy())
        assert server.metrics.cache_hits.value(model="m") == 1
        assert server.metrics.cache_misses.value(model="m") == 1


def test_different_inputs_are_not_confused(tmp_path: Path) -> None:
    with _server(_repo(tmp_path)) as server:
...
def test_a_hit_carries_this_request_s_identity_not_the_stored_one(tmp_path: Path) -> None:
...
def test_a_cached_response_cannot_be_mutated_by_its_caller(tmp_path: Path) -> None:
...
def test_eviction_is_bounded(tmp_path: Path) -> None:
    with _server(_repo(tmp_path)) as server:
...
def test_caching_is_off_by_default_and_costs_no_hash(tmp_path: Path) -> None:
