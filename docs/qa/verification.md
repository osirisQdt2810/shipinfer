...
| `no-fake, no-mock, verify-by-logging` | **HELD** | `grep` for `class .*Mock/Fake/Stub` in `src/` returns **nothing at all**: `backends/mock.py` was the standing exception and it is deleted (V148). The offline tier runs real TorchScript modules built to each config's own declared shapes (`tests/support/models.py`). Every benchmark number in this repository comes from real TensorRT engines on real GPUs. |
