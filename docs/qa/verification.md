...
| `no-fake, no-mock, verify-by-logging` | **PARTIAL** | `backends/mock.py` is deleted (V148) and the offline tier runs real TorchScript modules built to each config's own declared shapes (`tests/support/models.py`). `src/shipinfer/topology/elements/mock.py` still holds 14 `Mock*` element classes, registered in `topology/elements/__init__.py`: a chain must end in an OUTPUT element, so the loader's tests needed one that exists on a host with no accelerator until C8b registered `jsonlines`/`none`. Tracked as the elements follow-up (`v148-elements-part1..3`); this row returns to HELD when it lands. Every benchmark number in this repository comes from real TensorRT engines on real GPUs. |
