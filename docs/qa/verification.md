...
| `no-fake, no-mock, verify-by-logging` | **HELD** | `grep -rni mock src/` returns **nothing**: `backends/mock.py` and `topology/elements/mock.py` are both deleted (V148, parts 1-3), and no production comment cites a mock class any more. `grep -rn "impl: mock"` over `src/ tests/ model_repository/ topology/` → 0. Every element kind still has a real implementation — decode `[gstreamer, pyav, replay]`, detect/segment/embed `[pool]`, recognize `[pool, shipvision]`, track/mtmc `[shipvision]`, output `[jsonlines, kafka, none]`. The offline tier runs real TorchScript modules built to each config's own declared shapes (`tests/support/models.py`). Every benchmark number in this repository comes from real TensorRT engines on real GPUs. |
