...
namespace shipinfer {
    ...
    size_t MtmcStage::do_run(FrameState& state) {
        ...
        // roster is `cam-01..cam-04` while the bench fleet is `cam00..cam11`: no submit, no
        // instant, a null `global_id` on every object, and not one counter moving (#258 r1).
        // COUNTED when it does fire, because a routing mistake must not be silent either.
        if (routes_ && roster_.count(state.tag().camera_id) == 0) {
            barrier_->note_not_mine(state.tag().camera_id);
            ObjectBatch skipped;
