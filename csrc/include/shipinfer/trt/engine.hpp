...
class TrtInstance {
  public:
    ...
    // on the default stream and then called `cudaDeviceSynchronize`, which is device-wide —
    ...
    // needs to upload (a chunk's boxes). Persistent because `cudaMalloc` serialises on the
