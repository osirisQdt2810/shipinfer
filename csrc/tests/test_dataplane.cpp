...
void test_the_missing_stages_are_named() {
    ...
    FrameCollector collector(
        [&missing](FrameResult&& result) { missing = result.missing; }, 64, 1);
...
void test_the_letterbox_kernel_agrees_with_the_reference() {
    ...
    if (cudaMalloc(&device_src, host.size()) != cudaSuccess) {
        std::fprintf(stderr, "SKIP: no CUDA device for the parity tests\n");
        return;
    }
    cudaMalloc(&device_dst, static_cast<size_t>(3) * dst * dst * sizeof(float));
    cudaMemcpy(device_src, host.data(), host.size(), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(got.data(), device_dst, got.size() * sizeof(float), cudaMemcpyDeviceToHost);
    const auto want = letterbox_reference(host, src_h, src_w, dst, dst, true, 0.5f, map);

    double worst = 0;
    for (size_t i = 0; i < got.size(); ++i) worst = std::max(worst, static_cast<double>(std::fabs(got[i] - want[i])));
...
void test_a_degenerate_box_yields_a_black_crop() {
    ...
    uint8_t* device_src = nullptr;
    if (cudaMalloc(&device_src, host.size()) != cudaSuccess) return;
    cudaMemcpy(device_src, host.data(), host.size(), cudaMemcpyHostToDevice);

    const float boxes[8] = {4, 4, 12, 12, /* degenerate */ 5, 5, 5, 5};
    float* device_boxes = nullptr;
    float* device_dst = nullptr;
    cudaMalloc(&device_boxes, sizeof(boxes));
    cudaMalloc(&device_dst, static_cast<size_t>(2) * 3 * ch * cw * sizeof(float));
    cudaMemcpy(device_boxes, boxes, sizeof(boxes), cudaMemcpyHostToDevice);
