...
// twenty-four instances is thousands of lines of layer detail nobody reads, and on the Python
// side it was also the thing that deadlocked start-up.
...
void TrtEngine::introspect() {
    ...
    for (int i = 0; i < total; ++i) {
        ...
        if (batch < 0) {
            const auto profile_max = engine_->getProfileShape(
...
void TrtInstance::execute(int rows) {
    ...
    for (const auto& spec : engine_->inputs()) {
        ...
        if (!context_->setInputShape(spec.name.c_str(), dims)) {
            throw BackendError("setInputShape failed for " + spec.name + " at " +
                               std::to_string(rows) + " row(s) — a static plan refuses any "
                               "batch but its own");
        }
    }
    ...
    for (size_t i = 0; i < engine_->outputs().size(); ++i) {
        ...
        SHIPINFER_CUDA(cudaMemcpyAsync(host_outputs_[i].get(), output_buffers_[i].get(),
                                       spec.row_bytes() * static_cast<size_t>(rows),
                                       cudaMemcpyDeviceToHost, stream_));
    }
    SHIPINFER_CUDA(cudaStreamSynchronize(stream_));
    ++executed_;
