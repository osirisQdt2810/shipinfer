...
const char* to_string(FinishReason reason) {
    switch (reason) {
        case FinishReason::Complete: return "complete";
        case FinishReason::Incomplete: return "incomplete";
        case FinishReason::Timeout: return "timeout";
        case FinishReason::Shutdown: return "shutdown";
        case FinishReason::Evicted: return "evicted";
    }
    return "unknown";
}
...
bool FrameCollector::evict_locked() {
    ...
    const auto greediest = std::max_element(
        per_camera_.begin(), per_camera_.end(),
