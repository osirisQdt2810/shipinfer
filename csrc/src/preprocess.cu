...
#include "shipinfer/cuda/preprocess.hpp"
...
__global__ void letterbox_kernel(const uint8_t* src, int src_h, int src_w, float* dst, int dst_h,
                                 int dst_w, float scale, int pad_x, int pad_y, bool swap_rb,
                                 float pad_value) {
...
__global__ void crop_resize_kernel(const uint8_t* src, int src_h, int src_w, const float* boxes,
                                   int count, float* dst, int dst_h, int dst_w, bool swap_rb) {
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
    const int y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    // detector, and the answer is a black crop rather than a launch that reads out of bounds.
    // The Python parity test pins the same behaviour.
    if (box_w <= 0.f || box_h <= 0.f) {
        for (int c = 0; c < 3; ++c) out[c * plane + y * dst_w + x] = 0.f;
        return;
    }

    const float sx = x1 + (static_cast<float>(x) + 0.5f) * box_w / static_cast<float>(dst_w);
    const float sy = y1 + (static_cast<float>(y) + 0.5f) * box_h / static_cast<float>(dst_h);
    for (int c = 0; c < 3; ++c) {
        const int src_c = swap_rb ? (2 - c) : c;
...
// NV12: a full-resolution Y plane followed by a half-resolution interleaved UV plane. This is
// what NVDEC hands back, and converting it to BGR first would cost a 6 MB temporary per 1080p
// frame — at 1000 frames a second that is 6 GB/s of pure waste.
__device__ __forceinline__ void nv12_to_bgr(const uint8_t* nv12, int src_h, int src_w, int stride,
                                            float y_f, float x_f, float* bgr) {
...
__global__ void nv12_letterbox_kernel(const uint8_t* nv12, int src_h, int src_w, int stride,
                                      float* dst, int dst_h, int dst_w, float scale, int pad_x,
                                      int pad_y, bool swap_rb, float pad_value) {
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
    const int y = blockIdx.y * blockDim.y + threadIdx.y;
    if (x >= dst_w || y >= dst_h) return;
...
LetterboxMap letterbox_into(const uint8_t* src_device, int src_h, int src_w, float* dst_device,
    ...
    letterbox_kernel<<<grid, block, 0, stream>>>(src_device, src_h, src_w, dst_device, dst_h,
                                                 dst_w, map.scale, map.pad_x, map.pad_y, swap_rb,
                                                 pad_value);
    SHIPINFER_CUDA(cudaGetLastError());
    return map;
}
...
void crop_resize_into(const uint8_t* src_device, int src_h, int src_w, const float* boxes_device,
    ...
    crop_resize_kernel<<<grid, block, 0, stream>>>(src_device, src_h, src_w, boxes_device, count,
