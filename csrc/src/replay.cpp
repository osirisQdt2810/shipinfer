...
FrameLibrary::FrameLibrary(const std::string& folder, int limit) {
    ...
    for (const auto& path : paths) {
        ...
        decoded.pixels.assign(image.data,
                              image.data + static_cast<size_t>(image.total()) * image.elemSize());
    ...
    // of copies taking the slow path. `cudaHostRegister` on the decoded library is one call at
    ...
    for (auto& image : frames_) {
        if (cudaHostRegister(image.pixels.data(), image.pixels.size(), cudaHostRegisterDefault) !=
            cudaSuccess) {
            cudaGetLastError();  // cleared, so the next real error is not misattributed
            pinned_ = false;
        }
    }
...
FrameLibrary::~FrameLibrary() {
    if (!pinned_) return;
    for (auto& image : frames_) cudaHostUnregister(image.pixels.data());
}
...
ReplayCamera::~ReplayCamera() { stop(); }
