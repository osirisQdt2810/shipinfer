...
PerceptionGraph::PerceptionGraph(const GraphConfig& config) : config_(config) {
    ...
    if (!config.ship_embedder_plan.empty()) {
        ship_embedder_ = std::make_unique<ModelPool>("ship_embedder", config.ship_embedder_plan,
                                                     config.devices,
                                                     config.ship_embedder_instances);
    }
}
...
void PerceptionGraph::execute(std::vector<Work>& batch, int device,
    ...
    try {
        ...
        // then called `cudaDeviceSynchronize`, which waits for *every* context on the device:
...
void PerceptionGraph::run_objects(ModelPool* pool, FrameState& state,
    ...
    for (size_t start = 0; start < indices.size(); start += static_cast<size_t>(limit)) {
        ...
        try {
            ...
            // The instance's own scratch, not a fresh `cudaMalloc` per chunk: allocation
            ...
            SHIPINFER_CUDA(cudaMemcpyAsync(box_device, padded.data(),
                                           padded.size() * sizeof(float),
                                           cudaMemcpyHostToDevice, lease.instance->stream()));
