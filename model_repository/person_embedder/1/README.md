...
To build this one, on the node that will run it and inside the container:
    ...
    cp models/reid_r50_fp32.engine model_repository/person_embedder/1/model.plan

Two steps, because the `reid` target builds ONE engine that this model and its sibling
both use, and installs it into no version directory — `--only person_embedder` exits 2 with
"unknown model(s)".
