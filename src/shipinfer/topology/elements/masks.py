...
UNDER `topology/elements/`, not `pipeline/graph/`, since P6-SEGMENT-CROP: `PoolSegment` is the
element that needs it and `topology/` may not import `pipeline/`. `pipeline/graph/masks.py`
re-exports it, so the counting-simulation graph is unchanged -- one definition, imported the
one direction the layering allows.
