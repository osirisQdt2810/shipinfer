...
class KafkaResultSink(ResultSink):
    ...
    def _do_emit(self, event: PerceptionEvent) -> None:
        ...
        self._report_one_delivery_failure()
    ...
    def _on_delivery(self, error: Any, message: Any) -> None:
        ...
        camera = _camera_of(message)
        log = _LOG.error if failures == 1 or failures % _LOG_EVERY == 0 else _LOG.debug
        log(
            "kafka sink: broker refused a message for camera %s on topic %s (%s) "
    ...
    def _report_one_delivery_failure(self) -> None:
        """Charge one observed delivery failure to the frame being emitted right now.

        Raises:
            SinkDeliveryError: the broker refused a message. Which message is deliberately
                not claimed — it is an earlier one — but the *rate* is exact, which is what
                makes ``pipeline_sink_failures_total`` track publish loss one for one.
        """
        with self._delivery:
            if not self._unreported_failures:
                return
            self._unreported_failures -= 1
            error = self._last_delivery_error
        raise SinkDeliveryError(
            f"kafka did not deliver a message to topic {self.topic!r} on {self.brokers}: "
            f"{error}"
        )
    ...
    def flush(self, timeout_s: float = 5.0) -> None:
        ...
        with self._delivery:
            unreported, self._unreported_failures = self._unreported_failures, 0
            failures = self.delivery_failures
        if remaining:
            _LOG.warning("kafka sink closed with %d message(s) undelivered", remaining)
        if unreported:
            _LOG.error(
                "kafka sink: %d message(s) the broker refused were never charged to a "
                "frame (%d delivery failure(s) in total, last: %s)",
                unreported,
                failures,
                self._last_delivery_error,
            )
    ...
    def stats(self) -> dict[str, Any]:
        with self._delivery:
            ...
            failures = self.delivery_failures
            unreported = self._unreported_failures
        return {
            **super().stats(),
            "topic": self.topic,
            ...
            "unreported_delivery_failures": unreported,
        }
