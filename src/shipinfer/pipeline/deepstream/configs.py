...
def tracker_config(deepstream: DeepStreamSettings, *, pipeline: PipelineSettings) -> str:
    ...
    operator who wants NVIDIA's tuned numbers points ``runner.deepstream.tracker_config`` at
    the SDK's own ``config_tracker_NvDCF_perf.yml``, and this file is then not generated at all.
    """
    return (
        "%YAML:1.0\n"
        "# Generated by shipinfer; see runner.deepstream.tracker_config to use your own.\n"
        "BaseConfig:\n"
    ...
    *,
    ...
    if not deepstream.bbox_parser and not _is_coverage_bbox_pair(artifact.config.outputs):
        ...
        raise ConfigurationError(
            ...
            f"runner.deepstream.bbox_parser (nvinfer's `parse-bbox-func-name`) and "
...
def _class_ids_for(
    ...
    for label in labels:
        if label not in by_label:
            raise ConfigurationError(
                f"runner.deepstream.operate_on[{model!r}] names label {label!r}, which "
