...
held `rows` and `requests`, and the bench's per-device table gained a `(busy)` line. Requests
and rows say what a model was ASKED to do; only time says whether it could -- which redirected
the next optimisation away from the GPU (`NOT-GPU-BOUND-AT-FIVE-GPUS`): 68% of ceiling, and
four times the queue bought 1.8% more events with FLAT occupancy. It is wall time, not GPU
time, so an upper bound, and a thrown batch is not charged; both caveats are now asserted.
#175 fixed the divisor #170 had only caveated -- cumulative microseconds over `--seconds`,
which contains the warm-up, understated the steady window by ~7 points. Each shard divides by
its OWN window and `aggregate` unions them, sound because a shard IS a GPU.
**A counter's test has to fail on a wrong VALUE, not only on a missing line.** Deleting the
`+=` any suite catches; a second `/1000` turns 156% of ceiling into 0.2%. Floor and ceiling on
both planes, each checked against the corruption it guards.
