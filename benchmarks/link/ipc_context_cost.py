...
from the child, with the slab's own bytes subtracted on the owner side. The delta on the
OWNER's device is the foreign-context cost the opener pays there; the delta on the opener's
device is whatever the driver adds locally for the mapping.
