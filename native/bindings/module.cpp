...
NormalizeParams make_params(const std::vector<float> &mean,
...
public:
  ...
  py::tuple letterbox_into(const std::vector<U8Array> &images,
    ...
    stage_frames(images, dst_h, dst_w, scales.mutable_data(),
                 pads.mutable_data());
    ...
    const auto stream = reinterpret_cast<gpuStream_t>(stream_handle);
    {
      ...
      shipinfer::check(gpuSetDevice(device_index_), "gpuSetDevice");
      shipinfer::letterbox_batch(
          views_host_, reinterpret_cast<float *>(out_ptr), dst_h, dst_w, params,
  ...
  /// testing.
  py::tuple letterbox_batch(const std::vector<U8Array> &images, int dst_h,
                            int dst_w, const std::vector<float> &mean,
                            const std::vector<float> &std, bool swap_rb,
                            int pad_value, uintptr_t stream_handle) {
    const int batch = static_cast<int>(images.size());
    ...
    auto *device_out =
        static_cast<float *>(output_.reserve(elems * sizeof(float)));
    auto extras = letterbox_into(
        images, reinterpret_cast<uintptr_t>(device_out), elems * sizeof(float),
        dst_h, dst_w, mean, std, swap_rb, pad_value, stream_handle);
    download(device_out, result.mutable_data(), elems * sizeof(float),
             stream_handle);
  ...
  void crop_into(const U8Array &image, const F32Array &boxes, uintptr_t out_ptr,
    ...
    auto *device =
        static_cast<unsigned char *>(frames_.reserve(frame_bytes + box_bytes));
    const auto params = make_params(mean, std, swap_rb);
    const auto stream = reinterpret_cast<gpuStream_t>(stream_handle);
    {
      py::gil_scoped_release release;
      ...
      shipinfer::crop_batch(
          view, reinterpret_cast<const float *>(device + frame_bytes),
          num_boxes, reinterpret_cast<float *>(out_ptr), dst_h, dst_w, params,
          stream);
      shipinfer::check(gpuStreamSynchronize(stream), "crop synchronize");
    }
  }

  F32Array crop_batch(const U8Array &image, const F32Array &boxes, int dst_h,
                      int dst_w, const std::vector<float> &mean,
                      const std::vector<float> &std, bool swap_rb,
    ...
    auto *device_out =
        static_cast<float *>(output_.reserve(elems * sizeof(float)));
    crop_into(image, boxes, reinterpret_cast<uintptr_t>(device_out),
  ...
  py::array_t<int64_t> nms(const F32Array &boxes, const F32Array &scores,
                           ...
                           int max_output, uintptr_t stream_handle) {
    const auto box_info = boxes.request();
    ...
    std::vector<int64_t> kept;
    {
      ...
      kept = shipinfer::nms(static_cast<const float *>(box_info.ptr),
                            static_cast<const float *>(score_info.ptr), n,
                            iou_threshold, score_threshold, max_output,
                            reinterpret_cast<gpuStream_t>(stream_handle));
    }
    auto result = py::array_t<int64_t>(static_cast<py::ssize_t>(kept.size()));
    if (!kept.empty()) {
      std::memcpy(result.mutable_data(), kept.data(),
                  kept.size() * sizeof(int64_t));
    }
    return result;
  }
...
private:
  ...
  void download(const void *device_src, void *host_dst, size_t bytes,
    ...
    auto *staging = pinned_download_.reserve(bytes);
    {
      py::gil_scoped_release release;
      const auto stream = reinterpret_cast<gpuStream_t>(stream_handle);
      shipinfer::check(gpuMemcpyAsync(staging, device_src, bytes,
  ...
  void stage_frames(const std::vector<U8Array> &images, int dst_h, int dst_w,
    ...
    for (int i = 0; i < batch; ++i) {
      ...
      const float scale = std::min(static_cast<float>(dst_h) / h,
                                   static_cast<float>(dst_w) / w);
  ...
  DeviceScratch
      output_; ///< only used by the host-returning convenience entry points
  PinnedScratch pinned_; ///< host staging for uploads
  PinnedScratch
      pinned_download_; ///< host staging for the convenience downloads
...
PYBIND11_MODULE(_C, m) {
  m.doc() =
      "shipinfer native data plane: fused preprocessing and device-side NMS";
  ...
  py::register_exception<shipinfer::GpuError>(m, "GpuError",
                                              PyExc_RuntimeError);
  ...
  py::class_<ImageOps>(m, "ImageOps",
                       "Fused pre/post-processing kernels bound to one device.")
      ...
      .def("letterbox_batch", &ImageOps::letterbox_batch, py::arg("images"),
           py::arg("dst_h"), py::arg("dst_w"), py::arg("mean"), py::arg("std"),
           py::arg("swap_rb"), py::arg("pad_value") = 114,
           py::arg("stream") = 0,
           "Fused resize+pad+convert+normalise+NCHW, returned as numpy.")
      .def("letterbox_into", &ImageOps::letterbox_into, py::arg("images"),
           py::arg("out_ptr"), py::arg("out_bytes"), py::arg("dst_h"),
           py::arg("dst_w"), py::arg("mean"), py::arg("std"),
           py::arg("swap_rb"), py::arg("pad_value") = 114,
           py::arg("stream") = 0,
           "Same, written straight into a caller-owned device buffer. The fast "
           "path.")
      .def("crop_batch", &ImageOps::crop_batch, py::arg("image"),
           py::arg("boxes"), py::arg("dst_h"), py::arg("dst_w"),
           py::arg("mean"), py::arg("std"), py::arg("swap_rb"),
           py::arg("stream") = 0,
           "Extract and resize N boxes, returned as numpy.")
      .def("crop_into", &ImageOps::crop_into, py::arg("image"),
           py::arg("boxes"), py::arg("out_ptr"), py::arg("out_bytes"),
           py::arg("dst_h"), py::arg("dst_w"), py::arg("mean"), py::arg("std"),
           py::arg("swap_rb"), py::arg("stream") = 0,
           "Same, written straight into a caller-owned device buffer.")
      .def("nms", &ImageOps::nms, py::arg("boxes"), py::arg("scores"),
           py::arg("iou_threshold"), py::arg("score_threshold"),
           py::arg("max_output"), py::arg("stream") = 0,
