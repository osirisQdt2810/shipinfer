...
/// would dominate.
__device__ __forceinline__ float sample_bilinear(const unsigned char *img,
...
/// either way.
__global__ void letterbox_kernel(const ImageView *views, int batch,
                                 float *__restrict__ out, int dst_h, int dst_w,
                                 float m0, float m1, float m2, float s0,
                                 float s1, float s2, bool swap_rb,
                                 float pad_value) {
  ...
  for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < total;
    ...
    float v[3];
    if (local_x < 0 || local_y < 0 || local_x >= view.out_w ||
        local_y >= view.out_h) {
      ...
      const float src_y =
          (static_cast<float>(local_y) + 0.5f) * view.height / view.out_h -
          0.5f;
      const float src_x =
          (static_cast<float>(local_x) + 0.5f) * view.width / view.out_w - 0.5f;
      v[0] =
          sample_bilinear(view.data, view.height, view.width, src_y, src_x, 0);
      v[1] =
          sample_bilinear(view.data, view.height, view.width, src_y, src_x, 1);
      v[2] =
          sample_bilinear(view.data, view.height, view.width, src_y, src_x, 2);
    }
...
__global__ void crop_kernel(ImageView frame, const float *__restrict__ boxes,
                            int num_boxes, float *__restrict__ out, int dst_h,
                            int dst_w, float m0, float m1, float m2, float s0,
                            float s1, float s2, bool swap_rb) {
  ...
  for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < total;
    ...
    const float x1 = fminf(fmaxf(boxes[n * 4 + 0], 0.f),
                           static_cast<float>(frame.width - 1));
    const float y1 = fminf(fmaxf(boxes[n * 4 + 1], 0.f),
                           static_cast<float>(frame.height - 1));
    const float x2 = fminf(fmaxf(boxes[n * 4 + 2], 0.f),
                           static_cast<float>(frame.width - 1));
    const float y2 = fminf(fmaxf(boxes[n * 4 + 3], 0.f),
                           static_cast<float>(frame.height - 1));
    ...
    if (x2 > x1 && y2 > y1) {
      const float src_y =
          y1 + (static_cast<float>(y) + 0.5f) * (y2 - y1) / dst_h - 0.5f;
      const float src_x =
          x1 + (static_cast<float>(x) + 0.5f) * (x2 - x1) / dst_w - 0.5f;
      v[0] = sample_bilinear(frame.data, frame.height, frame.width, src_y,
                             src_x, 0);
      v[1] = sample_bilinear(frame.data, frame.height, frame.width, src_y,
                             src_x, 1);
      v[2] = sample_bilinear(frame.data, frame.height, frame.width, src_y,
                             src_x, 2);
    }
...
__global__ void nms_mask_kernel(const float *__restrict__ boxes, int n,
                                float threshold,
...
void letterbox_batch(const std::vector<ImageView> &images, float *out,
  ...
  check(gpuMemcpyAsync(views.get(), images.data(),
                       images.size() * sizeof(ImageView), gpuMemcpyHostToDevice,
                       stream),
        "upload image views");
  ...
  letterbox_kernel<<<blocks, kBlockSize, 0, stream>>>(
      views.get(), batch, out, dst_h, dst_w, params.mean[0], params.mean[1],
      params.mean[2], params.std[0], params.std[1], params.std[2],
      params.swap_rb, static_cast<float>(pad_value));
...
void crop_batch(const ImageView &frame, const float *boxes, int num_boxes,
  ...
  crop_kernel<<<blocks, kBlockSize, 0, stream>>>(
      frame, boxes, num_boxes, out, dst_h, dst_w, params.mean[0],
      params.mean[1], params.mean[2], params.std[0], params.std[1],
      params.std[2], params.swap_rb);
...
std::vector<int64_t> nms(const float *boxes_host, const float *scores_host,
  ...
  std::stable_sort(order.begin(), order.end(), [&](int64_t a, int64_t b) {
    return scores_host[a] > scores_host[b];
  });
  ...
  check(gpuMemcpyAsync(d_boxes.get(), sorted.data(),
                       sorted.size() * sizeof(float), gpuMemcpyHostToDevice,
                       stream),
        "upload nms boxes");
  ...
  nms_mask_kernel<<<grid, kNmsBlock, 0, stream>>>(d_boxes.get(), n,
                                                  iou_threshold, d_mask.get());
  check_launch("nms_mask_kernel");

  std::vector<unsigned long long> mask(static_cast<size_t>(n) * col_blocks);
  check(gpuMemcpyAsync(mask.data(), d_mask.get(),
                       mask.size() * sizeof(unsigned long long),
  ...
  for (int i = 0; i < n && static_cast<int>(keep.size()) < max_output; ++i) {
    ...
    const unsigned long long *row =
        mask.data() + static_cast<size_t>(i) * col_blocks;
...
bool gpu_available() { return device_count() > 0; }
