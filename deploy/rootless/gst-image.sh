...
#
# `libopencv-dev` is here for the C++ plane, not the Python one: `scripts/build_csrc.py` now
# builds the ingest sources *in this image*, because it is the only place with both GStreamer's
# headers and its plugins, and `csrc/shipinfer/ingest/sources/replay.cpp` includes OpenCV. The
# live image was extended by `docker run` + `docker commit` on 27 Aug to unblock that; this line
# is what makes a `FORCE=1` rebake produce the same image instead of one that has quietly lost
